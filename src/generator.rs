...
impl Generator {
    ...
    pub fn generate(&self, element: &TgElement) -> Result<String> {
        self.generate_element(element, self.mode)
    }
    
    fn generate_element(&self, element: &TgElement, mode: ParseMode) -> Result<String> {
        match element {
            TgElement::Text(text) => Ok(escape_text(text, mode)),
            ...
            TgElement::Strikethrough(elements) => {
                let content = self.generate_elements(elements, mode)?;
                match mode {
                    ParseMode::MarkdownV2 => Ok(format!("~~{}~~", content)),
            ...
            TgElement::Mention { username } => Ok(format!("@{}", username)),
            
            TgElement::MentionId { user_id, text } => {
                match mode {
                    ParseMode::MarkdownV2 => Ok(format!(
                        "[{}](tg://user?id={})",
                        escape_text(text, mode),
            ...
            TgElement::Hashtag(tag) => Ok(format!("#{}", tag)),
            
            TgElement::Command { name, args } => {
                if args.is_empty() {
                    Ok(format!("/{}", name))
                } else {
            ...
            TgElement::Emoji(emoji) => Ok(emoji.clone()),
            
            TgElement::CustomEmoji { emoji, id } => {
                match mode {
            ...
            TgElement::List(list) => self.generate_list(list, mode),
            
            TgElement::Table(table) => self.generate_table(table, mode),
            
            TgElement::Quote(elements) => {
                let content = self.generate_elements(elements, mode)?;
                match mode {
                    ParseMode::MarkdownV2 => Ok(format!(">{}",
            ...
            TgElement::Custom { formatter, value } => {
            ...
            TgElement::Group(elements) => self.generate_elements(elements, mode),
        }
    }
    
    fn generate_elements(&self, elements: &[TgElement], mode: ParseMode) -> Result<String> {
        elements
            .iter()
...
fn calculate_column_widths(rows: &[&[TableCell]], _mode: ParseMode) -> Result<Vec<usize>> {
    ...
    for row in rows {
        for (i, cell) in row.iter().enumerate() {
            if i < widths.len() {
                let content_len = cell.content.iter()
                    .map(|e| match e {
                        TgElement::Text(t) => t.len(),
                        _ => 0,
                    })
                    .sum::<usize>();
